...
class Lockdown:
        ...
        try:
            try:
                ...
                for m in mod_role_strings:
                    mod_roles.append(
                        discord.utils.get(ctx.message.guild.roles, name=m))
            except:
                mod_roles = ["owner of the king"]
    ...
    async def remove(self, ctx, server, role):
        ...
        try:
            ...
            await ctx.send(
                self.bot.bot_prefix +
                "Successfully removed {} from the list of mod roles on {}!".
                format(role, server))
